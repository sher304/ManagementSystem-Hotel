...
struct BookingListView: View {
    ...
    init(context: ModelContext, reservations: [Reservation]) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        HStack {
                            Image(systemName: "calendar")
                                .foregroundColor(.gray)
                                .font(.caption2)
                            Text("\(reservation.checkInDate, format: .dateTime.day().month()) - \(reservation.checkOutDate, format: .dateTime.day().month().year())")
                            .font(.caption2)
                            .foregroundColor(.gray)
                        }
                    }
